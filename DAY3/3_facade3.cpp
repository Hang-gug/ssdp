...
// 3차 API  : 다양한 분야의 프로그램을 구현하는데 사용하는 전형적인 절차를
//			  책임지는 계층
//			  내부적으로 2차 API 를 사용하게 된다.
//			  facade 계층

// 2차 API	: 객체지향. 1차 API 의 각 분야와 1:1로 매핑되는 한개의 책임을 가진
//					   클래스들(수백 ~ 수천개 클래스)

// 1차 API	: C 언어.  다양한 분야의 수백~수천개의 함수와 구조체

// OS		: windows, linux, android, ios => 모두 C언어